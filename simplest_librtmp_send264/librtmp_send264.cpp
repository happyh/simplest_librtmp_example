...
void RTMP264_Read(FILE *fp, FILE *fp_264, int chunksize) {
	while (ReadH264Packet(fp,fp_264, chunksize))
	{

	}
}

bool ReadH264Packet(FILE *fp, FILE *fp_264,int chunksize)
{
	unsigned char b1 = 0;
	static unsigned unsigned char timestamp[3] = {0};
	static unsigned char bodysize[3] = {0};
	static unsigned unsigned char TypeID = 0;
	static unsigned unsigned int streamid = 0;

	if (fread(&b1, 1, 1, fp) != 1) {
		return false;
	}

	unsigned char chunk_type = b1 >> 6;
	unsigned char chunk_stream_id = b1 & 0x3f;
	switch (chunk_type)
	{
	case 0: //12
		if (fread(&timestamp, 1, 3, fp) != 3) {
			return false;
		}
		if (fread(&bodysize, 1, 3, fp) != 3) {
			return false;
		}
		if (fread(&TypeID, 1, 1, fp) != 1) {
			return false;
		}
		if (fread(&streamid, 1, 4, fp) != 4) {
			return false;
		}
		break;
	case 1: // 8
		...
		break;
	case 2: //4 全部使用同一个chunk stream的上一个chunk的。适用于长度不变的步伐传输。
		if (fread(&timestamp, 1, 3, fp) != 3) {
			return false;
		}
		break;
	...
	unsigned char  ffffff[3] = { 0xff,0xff,0xff };
	if (memcmp(timestamp, ffffff,3) == 0) {
		unsigned int extended_timestamp;
		if (fread(&extended_timestamp, 1, 4, fp) != sizeof(extended_timestamp)) {
			return false;
		}
	}

	int nbodysize = 0;
	memcpy(((char*)&nbodysize) + 1, bodysize, 3);
	nbodysize = ntohl(nbodysize);
	printf("file pos:0x%x,bodysize:0x%x,TypeID:%d,timestamp:%02x%02x%02x,chunk_stream_id:%d\n", ftell(fp), nbodysize, TypeID, timestamp[0],
		timestamp[1], timestamp[2], chunk_stream_id);
	int buffersize = chunksize + 1;
	unsigned char* pbuffer = new unsigned char[buffersize];
	bool first = true;
	while (nbodysize > 0) {
		if (nbodysize > chunksize) {
			buffersize = chunksize;
		}
		else {
			buffersize = nbodysize;
		}
		if (!first) {
		...
		//printf("file pos:%x\n", ftell(fp));
		if (TypeID == 9) {
			static char nal[4] = { 0x0,0x0,0x0,0x1 };
			if (first) {
				static char spsheader[6] = { 0x17,0x00,0x00,0x00,0x00,0x01 };
				if (buffersize > 16 && memcmp(pbuffer, spsheader,6) == 0) {
					int i = 11;
					unsigned short sps_len = 0;
					memcpy(((char*)&sps_len), pbuffer+i, 2);
					sps_len = ntohs(sps_len);
					i += 2;

					fwrite(nal, 1, 4, fp_264);
					fwrite(pbuffer + i, 1, sps_len, fp_264);

					i += sps_len;
					i += 1; //body[i++] = 0x01;
					unsigned short pps_len = 0;
					memcpy(((char*)&pps_len), pbuffer + i, 2);
					pps_len = ntohs(pps_len);
					i += 2;
					fwrite(nal, 1, 4, fp_264);
					fwrite(pbuffer + i, 1, pps_len, fp_264);
					i += pps_len;

					if (buffersize != i) {
						printf("buffersize:%d,i:%d,pos:0x%x\n", buffersize,i, ftell(fp));
					}
				}
				else {
					fwrite(nal, 1, 4, fp_264);
					fwrite(pbuffer + 9, 1, buffersize - 9, fp_264);
				}
			}
			else if(pbuffer[0] == 0xc4){
				fwrite(pbuffer + 1, 1, buffersize - 1, fp_264);
			}
			else {
				printf("data error pbuffer[0]:0x%02x!\n", pbuffer[0]);
			}	
		}
		else
		{
			printf("rh8.TypeID == 0x%02x!\n", TypeID);
		}
		if (first) {
			nbodysize -= buffersize;
		}
		else
		{
			nbodysize -= (buffersize-1);
		}
		first = false;
	}
